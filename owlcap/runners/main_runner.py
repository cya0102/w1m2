...
class MainRunner:
    def __init__(self, args):
        ...
        if self.selection_strategy not in {
                'nll', 'geometric_vote', 'semantic_vote', 'bpse'}:
            raise ValueError(
    ...
    def _train_one_epoch(self, epoch, **kwargs):
        self._configure_bpse_trainability(epoch)
        self.model.train()
        ...
        for bid, batch in enumerate(self.train_loader, 1):
            ...
            loss, loss_dict = rec_loss(
                **output, num_props=self.model.num_props,
                proposal_weights=proposal_weights, **self.args['loss'])
            ...
            quality_loss, quality_loss_dict = bpse_loss(
                **output, **self.args['loss'])
            loss_dict.update(quality_loss_dict)
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                for bid, batch in enumerate(loader, 1):
                    ...
                    nll_proposal_score = nll_loss.view(bsz, num_props)
                    proposal_score = nll_proposal_score
                    event_score = output.get('event_score')
                    if self.selection_strategy == 'bpse':
                        quality_logits = output.get('bpse_quality_logits')
                        if quality_logits is None:
                            raise ValueError(
                                'bpse selector requires model.bpse.enabled=true')
                        proposal_score = -quality_logits
                    ...
                    raw_props_tensor = torch.stack([
                        torch.clamp(raw_center - raw_width / 2, min=0),
                        torch.clamp(raw_center + raw_width / 2, max=1),
                    ], dim=-1)
                    raw_props = raw_props_tensor.cpu().numpy()
                    nll_idx = nll_proposal_score.argsort(dim=-1)
                    nll_sorted_props = raw_props_tensor.gather(
                    ...
                    diagnostics = calculate_candidate_diagnostics(
                        raw_props=raw_props,
                        nll_sorted_props=nll_sorted_props,
                        gt=gt,
                        context_width=context_width)
                    for name, (value, count) in diagnostics.items():
                        diagnostics_logger[name].update(value, count)
                    if output.get('bpse_quality_logits') is not None:
                        quality = output['bpse_quality_logits'].view(
                            bsz, num_props).gather(index=idx, dim=-1)
                        for name, value in {
                                'bpse_top1_quality': quality[:, 0].mean(),
                                'bpse_top1_completeness': output[
                                    'bpse_completeness'].view(
                                        bsz, num_props).gather(
                                            index=idx, dim=-1)[:, 0].mean(),
                                'bpse_top1_purity': output[
                                    'bpse_purity'].view(
                                        bsz, num_props).gather(
                                            index=idx, dim=-1)[:, 0].mean(),
                                'bpse_top1_equivalence': output[
                                    'bpse_equivalence'].view(
                                        bsz, num_props).gather(
                                            index=idx, dim=-1)[:, 0].mean(),
                        }.items():
                            diagnostics_logger[name].update(value.item(), bsz)
    ...
    def _build_dataset(self):
        ...
        bpse_config = self.args['model']['config'].get('bpse', {}) or {}
        if bpse_config.get('enabled', False):
            unit_config = args.get('query_units', {}) or {}
            if not unit_config.get('enabled', False):
                raise ValueError(
                    'model.bpse.enabled=true requires dataset.query_units.enabled=true')
            expected_units = int(bpse_config.get('max_query_units', 12))
            actual_units = int(unit_config.get('max_units', 12))
            if expected_units != actual_units:
                raise ValueError(
                    'BPSE max_query_units ({}) must equal dataset max_units ({})'.format(
    ...
    def _configure_bpse_trainability(self, epoch):
        """Freeze the baseline during quality-head warm-up, if configured."""
        if not getattr(self.model, 'use_bpse', False):
            return
        bpse_config = self.args['model']['config'].get('bpse', {}) or {}
        warmup = int(bpse_config.get('quality_warmup_epochs', 2))
        if (not bpse_config.get('freeze_baseline_during_quality_warmup', True)
                or epoch > warmup):
            for parameter in self.model.parameters():
                parameter.requires_grad = True
            return
        for name, parameter in self.model.named_parameters():
            parameter.requires_grad = name.startswith('bpse_scorer.quality_head.')
    ...
    def _load_cpl_baseline(self, path):
        """Load V4 baseline weights into BPSE, allowing only bpse.* missing."""
        state_dict = torch.load(path, map_location='cpu')
        ...
        illegal_missing = [name for name in missing
                           if not name.startswith('bpse_scorer.')]
        if unexpected or mismatched or illegal_missing:
            raise ValueError(
                'incompatible CPL baseline: unexpected={}, mismatched={}, '
                'missing={}'.format(unexpected, mismatched, illegal_missing))
        self.model.load_state_dict(compatible, strict=False)
        self.num_updates = 0
        self.lr_scheduler.step_update(self.num_updates)
        info('initialize BPSE from CPL baseline {}; loaded {} tensors.'.format(
            path, len(compatible)))
...
def calculate_candidate_diagnostics(raw_props, nll_sorted_props, gt,
    ...
    # This is an evaluation-only upper bound for the fixed candidate set.
    # It never enters training or proposal selection.
    oracle = top_n_metric(raw_props[:, :max_top].transpose(1, 0, 2), gt)
    diagnostics['oracle_R{}_mIoU'.format(max_top)] = (
        oracle['mIoU'], batch_size)
    diagnostics['oracle_R{}_IoU@0.5'.format(max_top)] = (
        oracle['IoU@0.5'], batch_size)
