...
def test_baseline_config_locks_actual_outer_boundary():
    config = json.loads(
        (ROOT / "config/activitynet/baseline.json").read_text())
    proposal = config["model"]["config"]["proposal_generator"]
    assert proposal["boundary_mode"] == "outer"
    assert proposal["boundary_shrink"] == 0.0
    assert config["model"]["config"]["bpse"]["enabled"] is False
