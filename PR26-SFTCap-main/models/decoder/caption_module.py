...
class CaptionModule(nn.Module):
    ...
    def __beam_search(self, objects_feats, action_feats, caption_feats, objects_pending, action_pending, caption_pending, state):
        ...
        output_prob, state = self.decoder(objects_feats, action_feats, caption_feats, objects_pending, action_pending, caption_pending, it_embed, state)
        logprob = output_prob
    ...
    # def sample(self, objects, object_masks, feature2ds, feature3ds,average_imgs, is_sample_max=True):
    # def sample(self, objects, object_masks, feature2ds, average_imgs, is_sample_max=True):
    def sample(self, objects, object_masks, feature2ds, is_sample_max=True):
        beam_size = self.beam_size
        ...
        # objects_semantics, action_semantics, caption_semantics = self.forward_encoder(objects, object_masks, feature2ds, feature3ds,average_imgs)
        '''源代码'''
        # objects_feats, action_feats, caption_feats, \
        # objects_semantics, action_semantics, caption_semantics = self.forward_encoder(objects, object_masks, feature2ds)
        sft_num=3
        split_num =sft_num - 1
        ...
        if beam_size > 1:
            return self.sample_beam(S_visual_features[0], S_visual_features[1], S_visual_features[2],
                                               S_visual_features[3], S_visual_features[4],
                                               S_visual_features[5])

        state = self.get_rnn_init_hidden(bsz=batch_size, hidden_size=self.decoder.hidden_dim,device=device)
        # state = self.get_rnn_init_hidden(batch_size, device)
        seq, seq_probabilities = [], []

        for t in range(self.max_caption_len):
            if t == 0:
                it = S_visual_features[0].new(batch_size).fill_(self.sos_idx).long()
            elif is_sample_max:
            ...
            log_probabilities, state = self.decoder(S_visual_features[0], S_visual_features[1], S_visual_features[2],
                                               S_visual_features[3], S_visual_features[4],
                                               S_visual_features[5], it_embed, state)
