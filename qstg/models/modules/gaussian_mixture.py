...
class GaussianMixtureProposalGenerator(nn.Module):
    ...
    @staticmethod
    def _check_optional_shape(value, expected, name):
    ...
    def predict_components(self, global_feature, sequence_length,
        ...
        if center_logit_bias is not None:
            self._check_optional_shape(
                center_logit_bias, center_logits.shape, 'center_logit_bias')
            center_logits = center_logits + center_logit_bias
        if width_logit_bias is not None:
            self._check_optional_shape(
                width_logit_bias, width_logits.shape, 'width_logit_bias')
    ...
    def combine(self, centers, widths, component_weights,
                reconstructor_features, importance_logit_bias=None):
        ...
        if importance_logit_bias is not None:
            self._check_optional_shape(
                importance_logit_bias, scores.shape,
