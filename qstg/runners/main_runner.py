...
class MainRunner:
    def __init__(self, args):
        ...
        if self.selection_strategy not in {
                'nll', 'geometric_vote', 'semantic_vote', 'qstg'}:
            raise ValueError(
        ...
        qstg_enabled = bool(self.args.get('model', {}).get('config', {})
                            .get('qstg', {}).get('enabled', False))
        if self.selection_strategy == 'qstg' and not qstg_enabled:
            raise ValueError(
                "selection_strategy='qstg' requires model qstg.enabled=true")
        ...
        self._build_model()
        self.qstg_stage = self.args.get('qstg_stage', 'baseline')
        trainable_scope = self.args.get(
            'trainable_scope',
            self.args['model']['config'].get('trainable_scope', 'all'))
        if hasattr(self.model, 'set_trainable_scope'):
            self.model.set_trainable_scope(trainable_scope)
        self._report_trainable_scope(trainable_scope)
        if 'train' in args:
            self._build_optimizer()
            self.num_updates = 0

    def _report_trainable_scope(self, scope):
        trainable = [
            (name, parameter.numel())
            for name, parameter in self.model.named_parameters()
            if parameter.requires_grad]
        total = sum(count for _, count in trainable)
        info('Trainable scope {}: {} parameters.'.format(scope, total))
        if scope == 'qstg_only':
            unexpected = [
                name for name, _ in trainable
                if not (name == 'qstg' or name.startswith('qstg.'))]
            if unexpected:
                raise RuntimeError(
                    'qstg_only unexpectedly leaves trainable parameters: {}'
                    .format(unexpected[:10]))
    ...
    def _train_one_epoch(self, epoch, **kwargs):
        ...
        for bid, batch in enumerate(self.train_loader, 1):
            ...
            graph_loss, graph_loss_dict = qstg_loss(
                **output, num_props=self.model.num_props,
                qstg_epoch=epoch, qstg_stage=self.qstg_stage,
                **self.args['loss'])
            loss_dict.update(graph_loss_dict)
            legacy_loss = loss + rnk_loss + event_loss + mixture_loss
            # Stage A is a binding/quality warm-up. Keep legacy objectives in
            # the log for comparability, but do not backpropagate them through
            # QSTG's adapted proposal feature while the CPL trunk is frozen.
            if (self.qstg_stage == 'A'
                    and getattr(self.model, 'use_qstg', False)):
                loss = graph_loss
            else:
                loss = legacy_loss + graph_loss
            loss.backward()
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                for bid, batch in enumerate(loader, 1):
                    ...
                    output = self.model(
                        epoch=epoch,
                        mask_mode=self.args.get('eval_mask_mode', 'legacy'),
                        eval_seed=self.args.get('seed', 0),
                        **net_input)
                    bsz = len(durations)
                    ...
                    if self.selection_strategy == 'qstg':
                        quality = output.get('proposal_quality_logit')
                        analytic = output.get('proposal_analytic_score')
                        if quality is None or analytic is None:
                            raise RuntimeError(
                                'QSTG selector requires quality and analytic scores')
                        quality = quality.view(bsz, num_props)
                        analytic = analytic.view(bsz, num_props)
                        proposal_score = proposal_score - float(
                            self.args.get('qstg_quality_weight', 0.2)) \
                            * torch.sigmoid(quality)
                        proposal_score = proposal_score - float(
                            self.args.get('qstg_analytic_weight', 0.1)) * analytic
                    ...
                    refined_selected_props = None
                    refined_center = output.get('qstg_eval_center')
                    refined_width = output.get('qstg_eval_width')
                    if refined_center is not None and refined_width is not None:
                        refined_center = refined_center.view(bsz, num_props)
                        refined_width = refined_width.view(bsz, num_props)
                        refined_center = refined_center.gather(index=idx, dim=-1)
                        refined_width = refined_width.gather(index=idx, dim=-1)
                        refined_selected_props = torch.stack([
                            torch.clamp(refined_center - refined_width / 2, min=0),
                            torch.clamp(refined_center + refined_width / 2, max=1),
                        ], dim=-1).cpu().numpy()
                    ...
                    if refined_selected_props is not None:
                        refined_res = top_1_metric(
                            refined_selected_props[
                                np.arange(bsz), selected_index], gt)
                        for key, value in refined_res.items():
                            metrics_logger['Refined R@1,' + key].update(
                                value, bsz)
                        refined_top = top_n_metric(
                            refined_selected_props[:, :k].transpose(1, 0, 2), gt)
                        for key, value in refined_top.items():
                            metrics_logger['Refined R@%d,' % k + key].update(
                                value, bsz)

                    for name in (
                            'proposal_coverage', 'proposal_exclusivity',
                            'proposal_relation', 'proposal_connectivity',
                            'node_gate', 'binding_prob', 'proposal_quality_logit'):
                        value = output.get(name)
                        if value is not None and torch.is_tensor(value):
                            diagnostics_logger['qstg_' + name].update(
                                value.float().mean().item(), bsz)
    ...
    def _save_model(self, path):
        state_dict = {
            ...
            'completed_epoch': getattr(self, '_current_epoch', 0),
            'config': self.args,
    ...
    def _load_model(self, path):
        ...
        self._current_epoch = state_dict.get('completed_epoch', 0)
        self.qstg_stage = state_dict.get('qstg_stage', self.qstg_stage)
        info('load model from {}, num_updates {}.'.format(path, self.num_updates))

    def _load_baseline_weights(self, path):
        """Load shape-compatible baseline weights into a QSTG model.

        Only the newly introduced ``qstg.*`` keys may be absent.  This is
        intentionally separate from legacy V3 conversion, which resets the
        event branch and has different compatibility semantics.
        """
        state_dict = torch.load(path, map_location='cpu')
        ...
        for name, value in source.items():
            ...
            if target[name].shape != value.shape:
                mismatch.append('{} ({} -> {})'.format(
                    name, tuple(value.shape), tuple(target[name].shape)))
                continue
            compatible[name] = value
        missing = [name for name in target if name not in compatible]
        unexpected_missing = [
            name for name in missing if not name.startswith('qstg.')]
        if unexpected or mismatch or unexpected_missing:
            raise ValueError(
                'baseline warm-start incompatible: unexpected={}, '
                'shape_mismatch={}, missing={}'.format(
                    unexpected, mismatch, unexpected_missing))
        self.model.load_state_dict(compatible, strict=False)
        self.num_updates = 0
        self._current_epoch = 0
        self.lr_scheduler.step_update(0)
        info('initialize QSTG from baseline {}: loaded {}, missing QSTG {}.'
             .format(path, len(compatible), len(missing)))
