...
class BaseDataset(Dataset):
    ...
    def _validate_frame_features(self, frames_feat, vid):
        dataset_name = self.__class__.__name__
        try:
            frames_feat = np.asarray(frames_feat, dtype=np.float32)
        except (TypeError, ValueError) as exc:
            raise ValueError(
                '{} feature for video ID {!r} cannot be converted to '
                'float32'.format(dataset_name, vid)) from exc

        if frames_feat.ndim != 2:
            raise ValueError(
                '{} feature for video ID {!r} must be 2-D [T, 512], got '
                'shape {}'.format(dataset_name, vid, frames_feat.shape))
        if frames_feat.shape[0] == 0:
            raise ValueError(
                '{} feature for video ID {!r} is empty'.format(
                    dataset_name, vid))
        if frames_feat.shape[1] != 512:
            raise ValueError(
                '{} feature for video ID {!r} must have last dimension 512, '
                'got shape {}'.format(dataset_name, vid, frames_feat.shape))
        if not np.isfinite(frames_feat).all():
            raise ValueError(
                '{} feature for video ID {!r} contains non-finite values'.format(
                    dataset_name, vid))
        return frames_feat
