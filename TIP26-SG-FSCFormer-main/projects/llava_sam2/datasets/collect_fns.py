...
def video_lisa_collate_fn(instances: Sequence[Dict],
    ...
    has_scene_graphs = any(inst.get('scene_graphs') is not None for inst in instances)
    ...
    has_segcaption_eval = any(inst.get('segcaption_eval') is not None for inst in instances)
    ...
    for example in instances:
        ...
        if has_segcaption_eval:
            segcaption_eval.append(example.get('segcaption_eval', None))
        ...
        if has_scene_graphs:
            scene_graphs.append(example.get('scene_graphs', None))
        if has_points:
    ...
    if has_segcaption_eval:
        data_dict['segcaption_eval'] = segcaption_eval
