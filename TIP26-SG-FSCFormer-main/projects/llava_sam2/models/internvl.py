...
class InternVL_Slowfast(InternVL_V1_5):
    ...
    def __init__(self,
        ...
        llm_model_path = llm_model_path or model_path
        use_external_llm = os.path.abspath(llm_model_path) != os.path.abspath(model_path)
        ...
        if use_external_llm:
            ...
            print_log(
                f'Use external LLM from {llm_model_path} with InternVL visual '
                f'backbone from {model_path}.',
                logger='current')
        ...
        if model_split:
            ...
            self._load_visual_backbone(model_path)
            self.model.vision_model.to(dtype=torch.bfloat16)
            self.model.mlp1.to(dtype=torch.bfloat16)
        ...
        if use_external_llm:
            self.model.language_model = AutoModelForCausalLM.from_pretrained(
                llm_model_path,
                torch_dtype=torch.bfloat16,
                quantization_config=quantization if quantization_llm else None,
                trust_remote_code=True)
        ...
        if tokenizer.pad_token_id is not None:
            ...
            self.model.language_model.config.pad_token_id = tokenizer.pad_token_id
    ...
    def _load_visual_backbone(self, model_path):
        index_file = os.path.join(model_path, 'model.safetensors.index.json')
        if not os.path.exists(index_file):
            raise FileNotFoundError(
                f'Cannot find InternVL safetensors index: {index_file}')
        ...
        print_log(
            f'Loaded {len(visual_state_dict)} InternVL visual backbone tensors '
            f'from {model_path}; missing={len(missing_keys)}, '
    ...
    def _llm_forward(
        ...
        if labels is not None:
            ...
            valid_labels = shift_labels != loss_fct.ignore_index
            if valid_labels.any():
