...
def main():
    ...
    from projects.llava_sam2.hf.models.configuration_sg_fscformer import SGFSCFormerConfig
    from projects.llava_sam2.hf.models.modeling_sg_fscformer import SGFSCFormerModel

    internvl_config = SGFSCFormerConfig.from_pretrained(cfg.path)
    config_dict = internvl_config.to_dict()
    config_dict['auto_map'] = \
        {'AutoConfig': 'configuration_sg_fscformer.SGFSCFormerConfig',
         'AutoModel': 'modeling_sg_fscformer.SGFSCFormerModel',
         'AutoModelForCausalLM': 'modeling_sg_fscformer.SGFSCFormerModel'}

    config_dict["llm_config"]["vocab_size"] = len(model.tokenizer)
    config_dict["template"] = cfg.template
    sg_fscformer_hf_config = SGFSCFormerConfig(
        **config_dict
    )
    hf_sg_fscformer_model = SGFSCFormerModel(
        sg_fscformer_hf_config, vision_model=model.mllm.model.vision_model,
        language_model=model.mllm.model.language_model,
    )
    hf_sg_fscformer_model.load_state_dict(all_state_dict_new)

    hf_sg_fscformer_model.save_pretrained(args.save_path)
