...
<span class="highlighted-text" style='color:rgb(107, 100, 239)'>SG-FSCFormer</span>
"""
...
def process_markdown2(output_str, colors):
...
def show_mask_pred_video(video, masks, vis_dark=False):
    ret_video = []
    selected_colors = []
    colors = [(255, 0, 0), (0, 255, 0), (0, 0, 255),
              (255, 255, 0), (255, 0, 255), (0, 255, 255),
    ...
    for i_frame in range(len(video)):
        ...
        if vis_dark:
            ...
            mask_f = np.max(_mask_image, axis=2) > 0  # 创建二维掩码 (H, W)
            mask_f = mask_f[:, :, np.newaxis]  # 增加通道维度 (H, W, 1)
            image = image * (1 - mask_f) + mask_f * (image * 0.4 + _mask_image * 0.6)  # alpha = 0.7
...
def frames_to_video2(
    ...
    try:
        ...
        fourcc = cv2.VideoWriter_fourcc(*'X264')  # 有的平台也支持 'avc1'
...
def write_mask_frames(pred_mask_frames, save_dir):
    ''' Write masks to disk 
    Args:
    ...
    '''
    for t, pred_mask_frame in enumerate(pred_mask_frames):
        # save image frame
        img_dir = os.path.join(save_dir, "img_frames")
        os.makedirs(img_dir, exist_ok=True)
        img_save_path = os.path.join(img_dir, f"frame_{t}.jpg")
        cv2.imwrite(img_save_path, cv2.cvtColor(pred_mask_frame, cv2.COLOR_RGB2BGR)) 
