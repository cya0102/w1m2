...
def _get_rawvideo_dec_3(image_or_video_path, select_frames=10):
    video_frames_paths = os.listdir(image_or_video_path)
    video_frames_paths = [os.path.join(image_or_video_path, frame) for frame in video_frames_paths]
    N = len(video_frames_paths)
    if N < select_frames:
        M = select_frames - N
        # 为了防止列表中的图片数量不足 M，这里使用 cycle 对最后 min(M, N) 张图片进行循环
        last_images = video_frames_paths[-min(M, N):]  # 获取最后几张图片
        extra = list(islice(cycle(last_images), M))
        video_frames_paths.extend(extra)
    else:
        step = N // select_frames  # Number of frames to skip between selections
        video_frames_paths = [video_frames_paths[i * step] for i in range(select_frames)]
        
    images_paths = []
    for img_path in video_frames_paths:
    ...
    # images_paths = [images_paths] # B x T x (H x W x C) # Add batch dimension
    return images_paths
...
class VideoQADataset(Dataset):
    ...
    def dataset_map_fn(self, data_dict, select_k=5):
        ...
        if 'youcook2' in video_file:
            images = _get_rawvideo_dec_3(video_file, select_frames=select_k)
            if self.use_fast:
                fast_images = _get_rawvideo_dec_3(video_file, select_frames=self.n_fast_images)
            else:
                fast_images = None
        else:
