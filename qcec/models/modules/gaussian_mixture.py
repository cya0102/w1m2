...
class GaussianMixtureProposalGenerator(nn.Module):
    ...
    def predict_components(self, global_feature, sequence_length,
                           center_logit_bias=None, width_logit_bias=None):
        ...
        by proposal, matching ``component_counts``.  Optional logit biases are
        deliberately generic so the generator remains independent of QCEC.
        """
        ...
        if center_logit_bias is not None:
            if center_logit_bias.shape != center_logits.shape:
                raise ValueError(
                    "center_logit_bias must have shape {}".format(
                        tuple(center_logits.shape)))
            center_logits = center_logits + center_logit_bias.to(
                device=center_logits.device, dtype=center_logits.dtype)
        if width_logit_bias is not None:
            if width_logit_bias.shape != width_logits.shape:
                raise ValueError(
                    "width_logit_bias must have shape {}".format(
                        tuple(width_logits.shape)))
            width_logits = width_logits + width_logit_bias.to(
                device=width_logits.device, dtype=width_logits.dtype)
