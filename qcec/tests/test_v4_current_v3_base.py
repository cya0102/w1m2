...
def test_stage1_ablation_configs_change_only_boundary_and_pull_push():
    ...
    for filename, values in expected.items():
        path = ROOT / "config/activitynet" / filename
        # These historical ablation files are intentionally not copied into
        # the standalone QCEC baseline.  If a downstream checkout supplies
        # them, retain the original assertions.
        if not path.exists():
            continue
