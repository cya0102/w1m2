...
def test_none_logit_bias_preserves_generator_outputs():
    torch.manual_seed(21)
    generator = GaussianMixtureProposalGenerator(
        hidden_size=4, num_proposals=3, max_components=3)
    feature = torch.randn(2, 4)
    first = generator.predict_components(feature, 12)
    second = generator.predict_components(
        feature, 12, center_logit_bias=None, width_logit_bias=None)
    for lhs, rhs in zip(first, second):
        assert torch.equal(lhs, rhs)


def test_nonzero_logit_bias_has_expected_shape_and_gradient():
    generator = GaussianMixtureProposalGenerator(
        hidden_size=4, num_proposals=3, max_components=3)
    feature = torch.randn(2, 4, requires_grad=True)
    center_bias = torch.zeros(2, generator.total_components, requires_grad=True)
    width_bias = torch.zeros(2, generator.num_proposals, requires_grad=True)
    centers, widths, weights = generator.predict_components(
        feature, 12, center_logit_bias=center_bias,
        width_logit_bias=width_bias)
    (centers.sum() + widths.sum() + weights.sum()).backward()
    assert center_bias.grad is not None
    assert width_bias.grad is not None
    assert torch.isfinite(center_bias.grad).all()
    assert torch.isfinite(width_bias.grad).all()
